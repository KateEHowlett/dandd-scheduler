var scheduler = new Vue({
  ...
  data() {
    return {
      players: [],
      week: ["Monday","Tuesday","Wednesday","Thursday","Friday","Friday","Saturday","Sunday"],
      playerName:'',
      daysOfTheWeek: [
      ...
      if(this.selected!=''){
        for(let player of this.players){
          if(player.name===this.selected){
            ...
            for(let j=0;j<=2;j++){
              console.log("Round " + j)
              for(let i=0;i<this.daysOfTheWeek.length;i++){ //Monday, Tuesday, Wednesday etc.
                for(let key in this.daysOfTheWeek[i]){
                  if(this.daysOfTheWeek[i][key]=="true"){
                    console.log("wtf " + this.daysOfTheWeek[i].name + " " + key)
 ...
 console.log("Index: " + this.$refs.days[index]);
            console.log("Individuals/Index: " + this.$refs.individuals[i].$refs.days[index]);
        this.$refs.individuals[i].$refs.days[index].classList.remove("unknown","busy","tentative","session")
            this.$refs.days[index].classList.add("available");
          } else if (availabilityType=="Busy"){ this.$refs.days[index].classList.remove("unknown","available","tentative","session")
            this.$refs.days[index].classList.add("busy");
          } else if (availabilityType=="Tentative"){ this.$refs.days[index].classList.remove("unknown","busy","available","session")
            this.$refs.days[index].classList.add("tentative");
          } else if (availabilityType=="Session"){ this.$refs.days[index].classList.remove("unknown","busy","tentative","available")
            this.$refs.days[index].classList.add("session");
          } else if (availabilityType=="Unknown") { this.$refs.days[index].classList.remove("available","busy","tentative","session")
            this.$refs.days[index].classList.add("unknown");
          } else { this.$refs.days[index].classList.remove("available","busy","tentative","session","unknown")
          }
        }
      }
      console.log(index)
